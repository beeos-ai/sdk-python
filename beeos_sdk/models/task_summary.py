...
class TaskSummary(BaseModel):
    ...
    TaskSummary
    """ # noqa: E501
    task_id: StrictStr = Field(description="Stable task identifier (== underlying channel_id).")
    agent_id: StrictStr
    caller_id: Optional[StrictStr] = Field(default=None, description="The owner user id of this task. Always equal to the caller for tasks they themselves submitted; populated from the channel metadata so it survives across sessions. ")
    state: StrictStr = Field(description="Channel-level state: `active` while the task is open (the agent may still be working / paused at `input_required`), `closed` after terminal reply / cancel / timeout. NOT the derived `TaskStatus` — use `getTask` for that. ")
    close_reason: Optional[StrictStr] = Field(default=None, description="Populated when `state=closed`. Mirrors the underlying channel's `closed_reason` so SDKs can branch without re-pulling the message log. Canonical values include `completed`, `canceled`, `timeout`. ")
    metadata: Optional[Dict[str, StrictStr]] = None
    created_at: Optional[datetime] = None
    closed_at: Optional[datetime] = None
    deadline_at: Optional[datetime] = None
    __properties: ClassVar[List[str]] = ["task_id", "agent_id", "caller_id", "state", "close_reason", "metadata", "created_at", "closed_at", "deadline_at"]
