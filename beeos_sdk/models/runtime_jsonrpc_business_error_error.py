...
class RuntimeJSONRPCBusinessErrorError(BaseModel):
    """
    RuntimeJSONRPCBusinessErrorError
    """ # noqa: E501
    code: Annotated[int, Field(le=-32001, strict=True, ge=-32021)]
    message: StrictStr
    ...
    def message_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['METHOD_NOT_SUPPORTED', 'USE_AGENTS_API', 'CONTRACT_VERSION_MISMATCH', 'INSTANCE_OFFLINE', 'INSTANCE_EPOCH_MISMATCH', 'IDEMPOTENCY_CONFLICT', 'COMMAND_EXPIRED', 'OUTCOME_UNKNOWN', 'OPERATION_NOT_FOUND', 'OPERATION_NOT_CANCELLABLE', 'CONVERSATION_CLEARING', 'PROJECTION_PENDING', 'PROJECTION_BLOCKED', 'UNAUTHORIZED_TARGET', 'INVALID_INVOCATION', 'INVALID_EXECUTION_GRANT', 'UNAUTHORIZED_PUBLISHER', 'OPERATION_CANCELLED', 'HISTORY_GENERATION_MISMATCH', 'AGENT_DEFAULT_MODEL_NOT_CONFIGURED', 'RUNTIME_EXECUTION_FAILED']):
            raise ValueError("must be one of enum values ('METHOD_NOT_SUPPORTED', 'USE_AGENTS_API', 'CONTRACT_VERSION_MISMATCH', 'INSTANCE_OFFLINE', 'INSTANCE_EPOCH_MISMATCH', 'IDEMPOTENCY_CONFLICT', 'COMMAND_EXPIRED', 'OUTCOME_UNKNOWN', 'OPERATION_NOT_FOUND', 'OPERATION_NOT_CANCELLABLE', 'CONVERSATION_CLEARING', 'PROJECTION_PENDING', 'PROJECTION_BLOCKED', 'UNAUTHORIZED_TARGET', 'INVALID_INVOCATION', 'INVALID_EXECUTION_GRANT', 'UNAUTHORIZED_PUBLISHER', 'OPERATION_CANCELLED', 'HISTORY_GENERATION_MISMATCH', 'AGENT_DEFAULT_MODEL_NOT_CONFIGURED', 'RUNTIME_EXECUTION_FAILED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RuntimeJSONRPCBusinessErrorError from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RuntimeJSONRPCBusinessErrorError from a dict"""
        if obj is None:
            return None
