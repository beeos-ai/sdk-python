...
class UpdateAgentRequest(BaseModel):
    """
    Body for `PATCH /api/v1/agents/{agentId}` (P1-B). Whitelist intentionally narrow — `description` / `name` / `version` / `avatar_url` / `capabilities` / `skills` are owned by the pod-side `POST /api/v1/agents/sync` and would silently revert if patched here. The empty object and unknown properties are rejected. If both MCP key spellings are supplied, their boolean values must agree. 
    """ # noqa: E501
    visibility: Optional[StrictStr] = Field(default=None, description="Agent ACL bucket. `private` is owner-only; `public` lets any authenticated caller invoke; `unlisted` is invokable but absent from public listings; `marketplace` is catalog-visible. ")
    mcp_enabled: Optional[StrictBool] = Field(default=None, description="Whether the agent shows up under MCP Gateway's `tools/list`. Independent of `visibility` — an `unlisted` agent can still be MCP-exposed; a `public` agent without MCP is invoke-only. ", alias="mcpEnabled")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["visibility", "mcpEnabled", "mcp_enabled"]
    ...
    def visibility_validate_enum(cls, value):
        ...
        if value not in set(['private', 'unlisted', 'public', 'marketplace']):
            raise ValueError("must be one of enum values ('private', 'unlisted', 'public', 'marketplace')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "mcpEnabled": obj.get("mcpEnabled", obj.get("mcp_enabled"))
        })
