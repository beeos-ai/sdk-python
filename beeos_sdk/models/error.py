...
class Error(BaseModel):
    ...
    Error
    """ # noqa: E501
    type: Optional[StrictStr] = Field(default=None, description="RFC 9457 problem-type slug. Most SDK consumers key on `code` rather than `type`; this field is kept for structural parity with the wider apierror catalogue. ")
    code: StrictStr = Field(description="Stable machine-readable error code. See [docs/reference/errors.md](https://github.com/beeos-ai/beeos/blob/main/docs/reference/errors.md) for the complete catalogue. ")
    message: StrictStr = Field(description="Human-readable explanation; presentation-only, never key on this.")
    request_id: Optional[StrictStr] = Field(default=None, description="Per-request correlation id propagated end-to-end (also returned in the `X-Request-Id` response header). Include this value when reporting errors so the server-side trace can be located. ")
    param: Optional[StrictStr] = Field(default=None, description="Field path of the offending parameter for validation-style errors (`code` in `missing_param` / `invalid_param`). Empty for errors that don't reference a specific field. ")
    metadata: Optional[Dict[str, StrictStr]] = Field(default=None, description="Optional structured key-value hints for the client (retry strategy, related resource ids, etc.). Always a string-to-string map. ")
    __properties: ClassVar[List[str]] = ["type", "code", "message", "request_id", "param", "metadata"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['api_error', 'invalid_request_error', 'authentication_error', 'permission_error', 'rate_limit_error', 'not_found_error', 'conflict_error', 'validation_error']):
            raise ValueError("must be one of enum values ('api_error', 'invalid_request_error', 'authentication_error', 'permission_error', 'rate_limit_error', 'not_found_error', 'conflict_error', 'validation_error')")
        return value
