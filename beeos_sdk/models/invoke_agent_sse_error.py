...
class InvokeAgentSSEError(BaseModel):
    """
    Recoverable error frame. The stream still emits a final `done` with `is_error: true` after this (audit-v4 P1-1) so SDKs can branch on either signal. 
    """ # noqa: E501
    type: StrictStr
    code: Optional[StrictStr] = Field(default=None, description="Stable sentinel string code. Canonical values include `agent_not_found`, `agent_unavailable`, `agent_timeout`, `forbidden`, `invalid_param`, `internal`. ")
    status_code: Optional[StrictInt] = Field(default=None, description="HTTP-equivalent status code (so SDKs can branch on either field).")
    message: StrictStr = Field(description="Human-readable error detail.")
    __properties: ClassVar[List[str]] = ["type", "code", "status_code", "message"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of InvokeAgentSSEError from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of InvokeAgentSSEError from a dict"""
        if obj is None:
            return None
