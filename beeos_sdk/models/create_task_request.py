...
class CreateTaskRequest(BaseModel):
    ...
    CreateTaskRequest
    """ # noqa: E501
    message: StrictStr = Field(description="Initial user prompt. Wrapped into a `chat_message` envelope.")
    context_id: Optional[StrictStr] = Field(default=None, description="Optional existing conversation/channel ID to reuse instead of creating a fresh task channel. Rare; most callers omit this. ")
    deadline_ms: Optional[Annotated[int, Field(le=604800000, strict=True, ge=0)]] = Field(default=None, description="Task deadline in milliseconds. When > 0, the channel auto-closes with `status=timeout` if no terminal reply lands in this window. Max 7 days (server-side cap). ")
    idempotency_key: Optional[StrictStr] = Field(default=None, description="Forwarded to MS's `channel_messages` UNIQUE index. Re-submitting with the same key returns the original task_id rather than spawning a duplicate. Recommended for retry-prone callers. ")
    metadata: Optional[Dict[str, StrictStr]] = Field(default=None, description="Caller-supplied correlation tags (trace_id, business labels). Reserved chatinvoke routing keys (`protocol`, `caller_owner_id`, `target_agent_id`, `delivery_principal`) are overwritten by the gateway and stripped from the response. ")
    attachments: Optional[Annotated[List[AttachmentRef], Field(max_length=16)]] = Field(default=None, description="Optional list of files previously uploaded via [`POST /api/v1/files/presign-upload`](#operation/presignFileUpload). Resolved server-side and embedded in the initial chat_message envelope. ")
    __properties: ClassVar[List[str]] = ["message", "context_id", "deadline_ms", "idempotency_key", "metadata", "attachments"]
