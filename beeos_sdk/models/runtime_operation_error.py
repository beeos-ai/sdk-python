...
class RuntimeOperationError(BaseModel):
    """
    ...
    retry_mode: StrictStr = Field(alias="retryMode")
    context: Optional[RuntimeOperationErrorContext] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["code", "message", "operationId", "requestId", "effectState", "retryMode", "context"]
    ...
    def effect_state_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['failed', 'cancelled', 'expired', 'outcome_unknown', 'committed']):
            raise ValueError("must be one of enum values ('failed', 'cancelled', 'expired', 'outcome_unknown', 'committed')")
        return value

    @field_validator('retry_mode')
    def retry_mode_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['none', 'resume', 'resubmit']):
            raise ValueError("must be one of enum values ('none', 'resume', 'resubmit')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RuntimeOperationError from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RuntimeOperationError from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "retryMode": obj.get("retryMode"),
            "context": RuntimeOperationErrorContext.from_dict(obj["context"]) if obj.get("context") is not None else None
        })
