...
class RegisterTaskWebhookRequest(BaseModel):
    """
    RegisterTaskWebhookRequest
    """ # noqa: E501
    url: StrictStr = Field(description="Absolute HTTPS endpoint to POST callbacks to. The receiver gets the protocol-specific payload (OpenAPI: a task event envelope; A2A: JSON-RPC StreamResponse; MCP: notification). ")
    token: Optional[StrictStr] = Field(default=None, description="Optional bearer token sent as `Authorization: Bearer <token>` on each callback so the receiver can verify the call. ")
    secret: Optional[Annotated[str, Field(min_length=16, strict=True, max_length=256)]] = Field(default=None, description="P2-A — HMAC-SHA256 signing key. When set, the deliverer signs every callback body and emits `X-BeeOS-Signature: t=<unix>,v1=<hex>` where `hex = hmac_sha256(secret, t || \".\" || body)`. The receiver should re-run the same HMAC to authenticate; mismatched timestamps (±5 min recommended tolerance) should be rejected to defend against replay.  **Write-only**. Never echoed on Get / List responses; the `has_secret` boolean on `TaskWebhookResponse` is the only observable signal. Rotate by Set-ing a fresh value; passing `\"\"` on update preserves the existing secret (use DELETE + re-register to clear). ")
    __properties: ClassVar[List[str]] = ["url", "token", "secret"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RegisterTaskWebhookRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RegisterTaskWebhookRequest from a dict"""
        if obj is None:
            return None
