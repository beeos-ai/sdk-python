...
class TaskSSEEnd(BaseModel):
    """
    Body of the terminal `event: end` frame. SDKs treat any value as a clean shutdown signal; the `reason` field disambiguates whether the agent terminated (`task_terminal`), MS closed the upstream stream (`stream_closed`), or the conversation channel was deleted (`channel_closed`). 
    """ # noqa: E501
    reason: StrictStr
    ...
    def reason_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['task_terminal', 'stream_closed', 'channel_closed']):
            raise ValueError("must be one of enum values ('task_terminal', 'stream_closed', 'channel_closed')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TaskSSEEnd from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TaskSSEEnd from a dict"""
        if obj is None:
            return None
