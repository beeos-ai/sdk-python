...
class CreateMessageRequest(BaseModel):
    """
    Request body for `POST /api/v3/conversations/{cid}/messages`. Set `state=streaming` (default) for opening a streaming envelope that subsequent PATCHes will mutate; set `state=completed` for a single-shot terminal message.  Idempotency: the `Idempotency-Key` header IS the message_id. Strongly recommend setting a stable client-generated UUID. 
    """ # noqa: E501
    type: StrictStr = Field(description="Message intent. Defaults to `agent_reply` in the SDK.")
    reply_to: Optional[StrictStr] = None
    body: Optional[StrictStr] = None
    parts: Optional[List[Part]] = None
    state: Optional[MessageState] = None
    stop_reason: Optional[StopReason] = None
    require_subscriber: Optional[StrictBool] = Field(default=None, description="When true, the POST fails with 410 no_subscriber if no peer is online.")
    __properties: ClassVar[List[str]] = ["type", "reply_to", "body", "parts", "state", "stop_reason", "require_subscriber"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "require_subscriber": obj.get("require_subscriber")
        })
        return _obj
