...
class MessageEnvelope(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Stable v3 message id (== Idempotency-Key on POST).")
    conversation_id: StrictStr
    type: StrictStr = Field(description="Message intent. `agent_reply` for terminal model output; `chat_message` for inbound user/agent requests; custom strings for application-specific intents.")
    sender: StrictStr = Field(description="Publisher principal id (agent / user / service).")
    reply_to: Optional[StrictStr] = Field(default=None, description="Message id this envelope replies to. Required for request/reply matching via the Message Service `wait` primitive. ")
    body: StrictStr = Field(description="Cumulative main text. Empty string on parts-only envelopes.")
    parts: Optional[List[Part]] = None
    state: MessageState
    stop_reason: Optional[StopReason] = None
    content: Optional[Any] = None
    created_at: datetime
    updated_at: Optional[datetime] = Field(default=None, description="Server's last-write timestamp. Bumped on every successful PATCH (streaming or terminal). Equals `created_at` after the initial POST; diverges as subsequent PATCHes accumulate (per-token streaming snapshots and the final terminal transition). Use this for SSE replay checkpoints and client-side freshness decisions. ")
    __properties: ClassVar[List[str]] = ["id", "conversation_id", "type", "sender", "reply_to", "body", "parts", "state", "stop_reason", "content", "created_at", "updated_at"]
