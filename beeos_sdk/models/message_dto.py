...
class MessageDTO(BaseModel):
    ...
    body: Optional[StrictStr] = Field(default=None, description="Cumulative agent reply text. Mutated in place across PATCH updates while state == \"streaming\"; frozen when state transitions to a terminal value (`completed` / `failed` / `refused` / `cancelled`). ")
    state: Optional[MessageState] = Field(default=None, description="Envelope lifecycle (canonical vocabulary, see MessageState). Pre-v3 rows omit this field; v3 consumers should treat absent state as `completed`. ")
    stop_reason: Optional[StopReason] = Field(default=None, description="Terminal reason set when state becomes non-streaming (canonical vocabulary, see StopReason). Only meaningful when state ∈ {completed, failed, refused, cancelled}. ")
    parts: Optional[List[Part]] = Field(default=None, description="Structured part list for rich replies (thinking / tool_use / tool_result / file / source / custom). Empty / absent for plain text replies — consumers should render `body` only in that case. Discriminated union; see the Part schema for the per-type field set. ")
    updated_at: Optional[datetime] = Field(default=None, description="Server's last-write timestamp (RFC3339 with nanosecond precision, UTC). Equals `created_at` after the initial POST and is bumped on every successful PATCH (streaming snapshot updates AND terminal state transitions). Terminal rows are immutable, so `updated_at` freezes once `state` leaves `streaming`. SSE replay clients SHOULD use this to ack progress checkpoints; REST clients SHOULD use it to detect mid-stream progress vs `created_at`-only first reads. ")
    __properties: ClassVar[List[str]] = ["message_id", "offset", "type", "publisher_id", "in_reply_to", "payload", "created_at", "body", "state", "stop_reason", "parts", "updated_at"]
