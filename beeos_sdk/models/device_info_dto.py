...
class DeviceInfoDTO(BaseModel):
    """
    BYOD (bring-your-own-device) registration / static metadata for an instance's backing device, e.g. a user-provided phone.  Distinct from `ControlInfo` (the `/computer` + `/mobile` live control surface): this is the registered hardware snapshot and is readable even when the device is offline. `capabilities` here are device hardware / Agent-reported capability strings — semantically different from `InstanceDataDTO.capabilities` (the instance-level capability JSON declaration). 
    """ # noqa: E501
    type: StrictStr = Field(description="Device type (e.g. `android`, `windows`, `linux`, `macos`).")
    model: StrictStr = Field(description="Device model / hardware name.")
    os: StrictStr = Field(description="Operating system name.")
    os_version: StrictStr = Field(description="Operating system version.", alias="osVersion")
    width: StrictInt = Field(description="Registered screen width in device pixels.")
    height: StrictInt = Field(description="Registered screen height in device pixels.")
    density: StrictInt = Field(description="Registered screen density (DPI).")
    capabilities: List[StrictStr] = Field(description="Device hardware / Agent-reported capability strings. Semantically different from `InstanceDataDTO.capabilities` (an instance-level capability JSON object). ")
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Optional caller-/device-supplied extra metadata (raw JSON object). Omitted entirely when absent or malformed. ")
    __properties: ClassVar[List[str]] = ["type", "model", "os", "osVersion", "width", "height", "density", "capabilities", "metadata"]
