...
class TaskResponseAllOfData(BaseModel):
    """
    TaskResponseAllOfData
    """ # noqa: E501
    task_id: StrictStr
    agent_id: StrictStr
    caller_id: Optional[StrictStr] = None
    status: TaskStatus
    result: Optional[Any] = None
    error: Optional[StrictStr] = Field(default=None, description="Human-readable failure cause, present when `status=failed`.")
    metadata: Optional[Dict[str, StrictStr]] = None
    created_at: Optional[datetime] = None
    started_at: Optional[datetime] = Field(default=None, description="First time the agent emitted any streaming output.")
    completed_at: Optional[datetime] = Field(default=None, description="Timestamp the task reached a terminal state.")
    deadline_at: Optional[datetime] = None
    truncated: Optional[StrictBool] = Field(default=None, description="True when the server's message scan hit its internal cap (1000 messages) before reaching the end of the channel log. The reported `status` / `result` / `error` reflect only the scanned prefix; clients seeing this flag should follow up with `GET /tasks/{id}/events` (SSE) for the authoritative tail. Absent (false) on typical tasks. ")
    __properties: ClassVar[List[str]] = ["task_id", "agent_id", "caller_id", "status", "result", "error", "metadata", "created_at", "started_at", "completed_at", "deadline_at", "truncated"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TaskResponseAllOfData from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TaskResponseAllOfData from a dict"""
        if obj is None:
            return None
