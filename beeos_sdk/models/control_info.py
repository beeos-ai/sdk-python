...
class ControlInfo(BaseModel):
    """
    Live control surface for an instance: online status, OS, current screen geometry, and the supported action verbs. 
    """ # noqa: E501
    online: StrictBool = Field(description="Whether the device is currently reachable for control.")
    os: StrictStr = Field(description="Device OS (e.g. `android`, `linux`, `windows`, `macos`).")
    screen_width: StrictInt = Field(description="Current screen width in device pixels.")
    screen_height: StrictInt = Field(description="Current screen height in device pixels.")
    coordinate_system: StrictStr = Field(description="Always `absolute_pixels` (origin top-left).")
    supported_actions: List[StrictStr] = Field(description="Action verbs this device supports (e.g. `screenshot`, `tap`).")
    current_app: Optional[StrictStr] = Field(default=None, description="Foreground app / window, when known.")
    __properties: ClassVar[List[str]] = ["online", "os", "screen_width", "screen_height", "coordinate_system", "supported_actions", "current_app"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ControlInfo from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ControlInfo from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "current_app": obj.get("current_app")
        })
        return _obj
