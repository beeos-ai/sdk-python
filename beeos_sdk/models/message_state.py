...
class MessageState(str, Enum):
    """
    Lifecycle state of a v3 message envelope. `streaming` is the sole non-terminal state; everything else is immutable.  State transitions:   - `streaming -> completed`  (normal LLM/agent finish)   - `streaming -> failed`     (transport / model error)   - `streaming -> refused`    (content filter / agent decline)   - `streaming -> cancelled`  (user_stop / timeout) Once terminal, the row is immutable and any PATCH returns `410 Gone` with `code = message_terminal`. 
    """
    ...
    def from_json(cls, json_str: str) -> Self:
        """Create an instance of MessageState from a JSON string"""
