...
class InvokeAgentRequest(BaseModel):
    ...
    InvokeAgentRequest
    """ # noqa: E501
    message: StrictStr = Field(description="The message to send to the agent.")
    context_id: Optional[StrictStr] = Field(default=None, description="Optional conversation context ID for multi-turn conversations.")
    timeout_ms: Optional[Annotated[int, Field(le=115000, strict=True, ge=0)]] = Field(default=None, description="Per-request timeout in milliseconds. Default 120000 (2 minutes). Hard-capped at 115000 server-side so the response always has time to flush before the HTTP `WriteTimeout` (120s) fires. Values above 115000 are silently clamped; if the agent does not reply within the effective window the server returns `service_timeout` (HTTP 504) — see [docs/reference/errors.md](../../docs/reference/errors.md). ")
    idempotency_key: Optional[StrictStr] = Field(default=None, description="Optional caller-generated idempotency key forwarded to Message Service as the chat_message's `idempotency_key`. Retries that race the same key dedup at MS (UNIQUE index on `channel_messages`). When omitted, the gateway generates a fresh UUID. The same key also doubles as the `message_id` an agent must echo back as `in_reply_to` on its reply. ")
    metadata: Optional[Dict[str, StrictStr]] = Field(default=None, description="Opaque caller-controlled key/value pairs merged into the channel metadata (e.g. `trace_id`, `user_id`, business tags). Reserved routing keys (`protocol`, `caller_owner_id`, `target_agent_id`, `delivery_principal`) are silently scrubbed server-side and cannot be overridden by the caller. ")
    attachments: Optional[Annotated[List[AttachmentRef], Field(max_length=16)]] = Field(default=None, description="Optional list of files previously uploaded via [`POST /api/v1/files/presign-upload`](#operation/presignFileUpload). Each `file_id` is resolved server-side to a presigned download URL and embedded in the chat_message envelope so the receiving agent can fetch the bytes without further BeeOS auth. ")
    __properties: ClassVar[List[str]] = ["message", "context_id", "timeout_ms", "idempotency_key", "metadata", "attachments"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "attachments": [AttachmentRef.from_dict(_item) for _item in obj["attachments"]] if obj.get("attachments") is not None else None
        })
        return _obj
