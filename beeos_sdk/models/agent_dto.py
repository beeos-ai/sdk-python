...
class AgentDTO(BaseModel):
    ...
    default_model_id: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(default=None, description="Safe canonical provider/model selector with an optional tag. Runtime-local keys, credentials, endpoint configuration, and opaque high-entropy values are never exposed.", alias="defaultModelId")
    visibility: StrictStr
    capabilities: AgentCapabilitiesDTO
    conversation_transport: StrictStr = Field(description="Conversation/message transport selected by the agent directory.", alias="conversationTransport")
    created_at: Optional[datetime] = Field(default=None, alias="createdAt")
    updated_at: Optional[datetime] = Field(default=None, alias="updatedAt")
    __properties: ClassVar[List[str]] = ["id", "instanceId", "name", "description", "displayName", "status", "slug", "mcpEnabled", "defaultModelId", "visibility", "capabilities", "conversationTransport", "createdAt", "updatedAt"]

    @field_validator('default_model_id')
    def default_model_id_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^[a-z0-9][a-z0-9._-]{0,63}(?:\/[A-Za-z0-9][A-Za-z0-9._-]{0,127}){1,7}(?::[A-Za-z0-9][A-Za-z0-9._-]{0,63})?$", value):
            raise ValueError(r"must validate the regular expression /^[a-z0-9][a-z0-9._-]{0,63}(?:\/[A-Za-z0-9][A-Za-z0-9._-]{0,127}){1,7}(?::[A-Za-z0-9][A-Za-z0-9._-]{0,63})?$/")
        return value

    @field_validator('visibility')
    def visibility_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['private', 'public', 'marketplace', 'unlisted']):
            raise ValueError("must be one of enum values ('private', 'public', 'marketplace', 'unlisted')")
        return value

    @field_validator('conversation_transport')
    def conversation_transport_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['acp', 'message-service']):
            raise ValueError("must be one of enum values ('acp', 'message-service')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "capabilities": AgentCapabilitiesDTO.from_dict(obj["capabilities"]) if obj.get("capabilities") is not None else None,
            "conversationTransport": obj.get("conversationTransport"),
