...
class TaskSSEBackfillTruncated(BaseModel):
    """
    Body of an `event: backfill_truncated` frame on `/tasks/{taskId}/events` and `/conversations/{convId}/events`. Forwarded verbatim from Message Service (ADR-0022 §1.2 / R2): emitted when the client reconnected with `since=N` where N is older than the oldest entry still retained in MS's per-channel Redis Stream — chunks within that window have been evicted.  SDKs SHOULD react by either:   1. resuming from `oldest_redis_offset` (skipping the evicted      chunks but staying on the live stream), or   2. refetching the polling endpoint with      `?include_deltas=true` for a best-effort backfill of      whatever PG still has.  Either path keeps the consumer on a consistent offset sequence; ignoring the frame is also legal — clients that only care about terminal `agent_reply` envelopes lose nothing because the terminal row is never evicted. 
    """ # noqa: E501
    oldest_redis_offset: StrictInt = Field(description="Smallest offset still readable from the per-channel Redis Stream. Clients can resume from any `offset >= oldest_redis_offset` without missing chunks. ")
    since: StrictInt = Field(description="The cursor the client reconnected with (echoed back for client-side correlation). ")
    hint: Optional[StrictStr] = Field(default=None, description="Human-readable English string explaining the gap. Informational only — SDKs should not pattern-match on it. ")
    __properties: ClassVar[List[str]] = ["oldest_redis_offset", "since", "hint"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TaskSSEBackfillTruncated from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TaskSSEBackfillTruncated from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "oldest_redis_offset": obj.get("oldest_redis_offset"),
            "since": obj.get("since"),
            "hint": obj.get("hint")
        })
        return _obj
