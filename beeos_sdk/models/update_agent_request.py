...
class UpdateAgentRequest(BaseModel):
    ...
    """ # noqa: E501
    visibility: Optional[StrictStr] = Field(default=None, description="Agent ACL bucket. `private` is owner-only; `public` lets any authenticated caller invoke; `unlisted` is invokable but absent from public listings. The `org` bucket is reserved for P1-C when the org-scope ACL lands. ")
    mcp_enabled: Optional[StrictBool] = Field(default=None, description="Whether the agent shows up under MCP Gateway's `tools/list`. Independent of `visibility` — an `unlisted` agent can still be MCP-exposed; a `public` agent without MCP is invoke-only. ")
    __properties: ClassVar[List[str]] = ["visibility", "mcp_enabled"]
    ...
    def visibility_validate_enum(cls, value):
        ...
        if value not in set(['private', 'unlisted', 'public']):
            raise ValueError("must be one of enum values ('private', 'unlisted', 'public')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "mcp_enabled": obj.get("mcp_enabled")
        })
        return _obj
