...
class WebhookDeliveryResponse(BaseModel):
    ...
    task_id: StrictStr
    renderer: StrictStr = Field(description="Payload format used at delivery time. OpenAPI callbacks registered through this gateway use the `openapi` renderer (TaskEvent envelope). ")
    status: StrictStr = Field(description="Lifecycle of this attempt. * `pending` — queued for first attempt or awaiting retry * `succeeded` — receiver returned 2xx (terminal) * `failed` — last attempt errored / non-2xx, will be   retried per the backoff schedule (intermediate) * `dead_letter` — exhausted retry schedule (terminal).   Recoverable via `POST .../redeliver`. ")
    attempt_num: Annotated[int, Field(strict=True, ge=0)] = Field(description="1-indexed count of attempts already made. New rows surface as 0 until the dispatcher claims them. ")
    last_response_status: Annotated[int, Field(strict=True, ge=0)] = Field(description="HTTP status of the most recent attempt. `0` means \"never attempted\" (transport or pre-flight error before any HTTP response). ")
    last_error: Optional[StrictStr] = Field(default=None, description="Truncated copy (≤ 2 KiB) of the most recent failure reason — either a transport error (`transport: connection refused`) or `http_status: <code>`. Empty on success / never attempted. ")
    next_attempt_at: datetime = Field(description="Scheduled time for the next attempt while `status` is `pending`. On terminal rows this carries the last scheduled value (no longer meaningful for retries). ")
    last_attempted_at: Optional[datetime] = None
    created_at: datetime
    completed_at: Optional[datetime] = None
    __properties: ClassVar[List[str]] = ["delivery_id", "webhook_id", "task_id", "renderer", "status", "attempt_num", "last_response_status", "last_error", "next_attempt_at", "last_attempted_at", "created_at", "completed_at"]

    @field_validator('renderer')
    def renderer_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['openapi', 'a2a', 'generic', 'mcp']):
            raise ValueError("must be one of enum values ('openapi', 'a2a', 'generic', 'mcp')")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pending', 'succeeded', 'failed', 'dead_letter']):
            raise ValueError("must be one of enum values ('pending', 'succeeded', 'failed', 'dead_letter')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "attempt_num": obj.get("attempt_num"),
