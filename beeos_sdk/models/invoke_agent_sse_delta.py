...
class InvokeAgentSSEDelta(BaseModel):
    """
    InvokeAgentSSEDelta
    """ # noqa: E501
    type: StrictStr
    text: StrictStr = Field(description="Streaming chunk text. Concatenate in order for the full reply.")
    __properties: ClassVar[List[str]] = ["type", "text"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['delta']):
            raise ValueError("must be one of enum values ('delta')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of InvokeAgentSSEDelta from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of InvokeAgentSSEDelta from a dict"""
        if obj is None:
            return None
