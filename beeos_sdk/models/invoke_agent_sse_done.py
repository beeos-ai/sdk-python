...
class InvokeAgentSSEDone(BaseModel):
    """
    Terminal frame. Clients should release the SSE connection on this frame. `is_error: true` may be paired with a populated `error` / `code` carrying the same payload as the preceding `InvokeAgentSSEError` frame so single-frame consumers can handle errors without buffering history. 
    """ # noqa: E501
    type: StrictStr
    text: Optional[StrictStr] = Field(default=None, description="Concatenated full reply text (echoes the deltas).")
    context_id: Optional[StrictStr] = Field(default=None, description="Conversation context ID (channel ID).")
    is_error: Optional[StrictBool] = Field(default=None, description="Whether the reply terminated in an error.")
    error: Optional[StrictStr] = Field(default=None, description="Human-readable failure cause (only when `is_error=true`).")
    code: Optional[StrictStr] = Field(default=None, description="Stable sentinel code (only when `is_error=true`). Matches the `code` on the preceding `InvokeAgentSSEError` frame. ")
    __properties: ClassVar[List[str]] = ["type", "text", "context_id", "is_error", "error", "code"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of InvokeAgentSSEDone from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of InvokeAgentSSEDone from a dict"""
        if obj is None:
            return None
