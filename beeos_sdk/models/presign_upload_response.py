...
class PresignUploadResponse(BaseModel):
    ...
    """ # noqa: E501
    file_id: StrictStr = Field(description="Stable identifier — attach via `attachments[].file_id`.")
    upload_url: StrictStr = Field(description="Presigned PUT URL. Client uploads bytes directly with the same `Content-Type` supplied in the request. Single-use; expires per storage-backend policy. ")
    download_url: Optional[StrictStr] = Field(default=None, description="Presigned GET URL for the same file, when the storage backend supports it (S3 / MinIO). Empty for backends that don't presign downloads — fall back to `GET /api/v1/files/{id}` in that case. ")
    download_expires_in: Optional[StrictInt] = Field(default=None, description="TTL of `download_url` in seconds.")
    __properties: ClassVar[List[str]] = ["file_id", "upload_url", "download_url", "download_expires_in"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PresignUploadResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PresignUploadResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "download_expires_in": obj.get("download_expires_in")
        })
        return _obj
